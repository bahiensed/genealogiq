...
Last session: 26/07/2026 — **Redesign do sistema de quotas/assinaturas do APP (passo 1: números + Dialog de limite) + página `/install` de instalação em 1 clique + cooldown/opt-out do popup PWA** (mesma branch `feat/app-geolocations-plural`, continuação — nada commitado ainda). Plano do redesign de quotas completo em `~/.claude/plans/eu-quero-que-voc-stateless-nebula.md` (sobrescreveu o plano do sign-wall, já concluído); o trabalho de PWA install foi pedido à parte, sem plan-mode, no mesmo dia.
...
- **`src/app/install/page.tsx` + `src/components/install-client.tsx`** (novos) — página dedicada que mostra o botão "Instalar" IMEDIATAMENTE (sem os 3s do popup flutuante); 1 clique já dispara o prompt nativo no Chrome/Edge/Android. 5 estados visuais: `checking` (~1.5s esperando o `beforeinstallprompt`) → `installed` | `native` | `ios` (passos na tela, sem esperar evento que nunca chega) | `unsupported` (Firefox etc. — mostra orientação em vez de nada).
...
- **Pendências**: (1) nada commitado — branch já acumulava Places + árvore + auth + footer + Documentos + o redesign de quotas, agora + este trabalho de PWA; vale reorganizar em commits/PRs separados antes de abrir; (2) verificação visual ao vivo do redesign de quotas (Douglas testa as 7 quotas excedendo cada uma, confirma texto/CTA do dialog, confirma que guardião Premium vê memoriais com quota Premium sem venda própria por memorial); (3) verificação visual ao vivo de `/install` (Chrome/Edge desktop + Android real, Safari iOS real, e Firefox pro estado `unsupported`) e do cooldown de 48h/checkbox permanente; (4) **passo 2 do pedido original de quotas** — repensar o fluxo de upgrade/`/subscriptions` em si — não iniciado; (5) a compra avulsa (Geo extra/QR extra/Memorial extra) mencionada no dialog de limite ainda não existe de verdade, só o texto que a antecipa.
