...
- **Preview + muro (REDESENHADO após feedback)**: memorial é essencialmente **público** pra anon — bio completa, galeria (até 12), homenagens (até 10, read-only), e **árvore + geolocalização também visíveis**. Em vez do card inline com gradient-fade (que quebrava o design da bio/galeria), um **Dialog modal** (`components/auth/signup-prompt.tsx`, client) surge **por cima** da página **no scroll** (segunda dobra: `scrollY > innerHeight*0.9`), dispara uma vez, dismissível. Queries anon bounded: `getGalleryByUserId(id, 12)`+`getGalleryCounts()`, `getApprovedTributesByProfileId(id, 10)`+`getTributeCountByProfileId()`.
- **Tree/geo públicos pra anon**: `tree/page.tsx` e `geolocation/page.tsx` usam `auth()` + `assertPublicMemorialAccess` (não mais `verifySession`). Tree passa `sessionUserId=""`, `canManage=false` pro `FamilyTreeCanvas` (read-only; as flags de edição são só UI — as writes ficam bloqueadas server-side por `verifySession()` em `actions/family-tree.actions.ts` + `guardian.actions.ts`, confirmado pelo reviewer). Cards tree/geo sempre linkam.
...
- Chaves i18n: `Auth.signupWallTitle/Description` (reusadas pelo dialog). Boundary files do `(public)` (loading/error/not-found) via `bruna-boundary-files`. **Removidos na rework**: `signup-wall.tsx`, `signup-gate.tsx`, `lib/bio-excerpt.ts` + chaves `signupGate*`/`notNow`/`lockedMetric`.
