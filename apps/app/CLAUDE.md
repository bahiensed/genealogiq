...
- **Commits**: tudo desta sessão (quotas + PWA install/popup + banner + fix da logo) já commitado em `main` local, 5 commits semânticos (`feat`/`fix`/`docs`), nada pushed ainda pro `origin/main`.
- **Pendências**: (1) `git push` quando Douglas quiser sincronizar com o remoto; (2) verificação visual ao vivo do redesign de quotas (Douglas testa as 7 quotas excedendo cada uma, confirma texto/CTA do dialog, confirma que guardião Premium vê memoriais com quota Premium sem venda própria por memorial); (3) verificação visual ao vivo de `/install` + banner em `/home` (Chrome/Edge desktop + Android real, Safari iOS real, Firefox pro estado `unsupported`) e do cooldown de 48h/checkbox permanente; (4) **passo 2 do pedido original de quotas** — repensar o fluxo de upgrade/`/subscriptions` em si — não iniciado; (5) a compra avulsa (Geo extra/QR extra/Memorial extra) mencionada no dialog de limite ainda não existe de verdade, só o texto que a antecipa.
