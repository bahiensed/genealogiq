...
beforeEach(() => {
  ...
  txMock.qrInventory.upsert.mockResolvedValue({})
})
...
describe("applyCheckoutSession — fulfillment", () => {
  ...
  it("PHYSICAL: creates one license per unit (pkg.quantity × order quantity) and no inventory", async () => {
    prismaMock.package.findUnique.mockResolvedValue({ quantity: 3, type: "PHYSICAL" })
    ...
    expect(txMock.qrInventory.upsert).not.toHaveBeenCalled()
  })

  it("DIGITAL: increments the tenant inventory by the total units and issues no licenses", async () => {
    prismaMock.package.findUnique.mockResolvedValue({ quantity: 5, type: "DIGITAL" })

    await applyCheckoutSession(session, ctx) // 5 × 2 = 10 units

    expect(txMock.qrInventory.upsert).toHaveBeenCalledWith({
      where:  { tenantId: "t1" },
      create: { tenantId: "t1", quantity: 10 },
      update: { quantity: { increment: 10 } },
    })
    expect(txMock.physicalQrLicense.createMany).not.toHaveBeenCalled()
  })

  it("is idempotent: a duplicate session (P2002) is swallowed without throwing", async () => {
    prismaMock.package.findUnique.mockResolvedValue({ quantity: 1, type: "DIGITAL" })
